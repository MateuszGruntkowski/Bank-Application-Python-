...
export default function LoginPage({ onLogin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      api.setUser({ username: data.username, is_admin: data.is_admin, user_id: data.user_id });
      onLogin({ username: data.username, is_admin: data.is_admin, user_id: data.user_id });
      navigate("/");
    } catch (err) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-primary-50 via-white to-primary-100">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 mt-2">{config.bankTagline}</p>
        </div>
